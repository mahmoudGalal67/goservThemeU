...
function FavButton({ active }) {
  ...
  useEffect(() => {
    if (favButton.current) {
      const handleClick = function () {
        if (favButton.current.classList.contains("deactivate")) {
          favButton.current.classList.remove("deactivate");
        }
        if (favButton.current.classList.contains("active")) {
          favButton.current.classList.add("deactivate");
        }
        favButton.current.classList.toggle("animate");
        favButton.current.classList.toggle("active");
        favButton.current.classList.toggle("inactive");
      };

      favButton.current.addEventListener("click", handleClick);
    }
  }, []);
  ...
  return (
    <a
      ref={favButton}
      className={`fav-button one inactive mobile button--secondary ${
        active ? "active animate" : "deactivate"
      }`}
    >
